...
class Simulator {
    ...
    public String simulate() {
        Shop shop = new Shop(this.numServers, this.numSelfChecks,
                this.restTimes, this.qmax);
        ...
        for (int i = 0; i < arrivalTimes.size(); i++) {
            ...
            events = events.add(new ArrivalEvent(newCustomer));
        }

        String log = "";
        while (!events.isEmpty()) {
            // get the next event and execute it
            Pair<Event, PQ<Event>> pair = events.poll();
            Event event = pair.first();
            events = pair.second();
            Pair<ImList<Event>, Shop> execResultPair = event.execute(shop);
            // add newly generated events to the queue
