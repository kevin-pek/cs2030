...
class BusRoutes {
    final BusStop stop;
    final String name;
    final CompletableFuture<Map<BusService,CompletableFuture<List<BusStop>>>> services;
    ...
    BusRoutes(BusStop stop, String name,
            CompletableFuture<Map<BusService,CompletableFuture<List<BusStop>>>> services) {
        this.stop = stop;
        this.name = name;
    ...
    public CompletableFuture<String> description() {
        return services.thenCompose(x -> {
            ...
            for (BusService service : x.keySet()) {
                output = output.thenCombine(x.get(service),
                        (s, stops) -> s + describeService(service, stops));
            }
            return output;
        });
    }
